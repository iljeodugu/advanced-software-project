...
namespace Melon1
{
    public partial class Form1 : Form
    {
        ...
        private void serverStop()//체크해서 서버 닫아주자
        {
            m_bCounnect = false;
            m_bStop = false;
        ...
        private void serverRun()
        {
            try
            {
                ...
                while (m_bStop)
                {
                    ...
                    if(first_Check)//처음인지 확인을해서 쿼리를 날려주자
                    {
                        first_Check = false;
        ...
        private void sendFile(string requestFileName)//파일보내는 함수
        {
            ...
            for (i = 0; i< (int)fi.Length/1024 ; )
            {
                if (is_OK)
                {
                    ...
                    m_Stream.Write(byteSendData, 0, byteSendData.Length);
                    m_Stream.Flush();
                    is_OK = false;
                    i++;
                }
            ...
            filestream.Read(byteSendData, 0, (int)fi.Length%1024);
            m_Stream.Write(byteSendData, 0, (int)fi.Length % 1024);
            m_Stream.Flush();
            is_OK = false;
