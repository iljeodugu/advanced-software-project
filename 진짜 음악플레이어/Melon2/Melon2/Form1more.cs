...
namespace Melon2
{
    public partial class Form1 : Form
    {
        ...
        string uplodaFileName = null;

        WindowsMediaPlayer WMP;//윈도우 플레이를 위한 자료들
        IWMPMedia Media;
        IWMPPlaylist PlayList;
        bool musicPlay = false;
        Array MediaSaver;//미디어 파일은 인터페이스라서 어레이에 저장을 해야겠다.

        public NetworkStream m_Stream;//스트림 라인
        ...
        bool musicThread_alive = false;//스레드 살아있는지 확인
        ...
        {
            this.serverMusicList.View = View.Details;//리스트뷰 초기화
        ...
        private void button2_Click(object sender, EventArgs e)//find path 키
        {
            ...
            try
            {
                ...
                while (m_bCounnect)
                {
                    ...
                    if (szMessage != null)
                    {
                        if (szMessage == "End Query")
                            break;
                        string[] temp = new string[4];//날아온것을 @를 기준으로 잘라서 파일을 만들자
                ...
                while (m_bCounnect)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (szMessage != null)
                        {
                            if (szMessage.IndexOf("Sending File") != -1)//만약에 sending file이 추가되어있으면 사이즈 받자
                            {
                    ...
                    progressBar1.Step =2;
                    fs = new FileStream(musicPath, FileMode.Append, FileAccess.Write);

                    for (i = 0; i < (int)filesize / 1024;)//파일크기만큼 반복문
                    {
                    ...
                    m_Stream.Read(response, 0, (int)filesize%1024);
                    fs.Write(response, 0, (int)filesize % 1024);
                    
                    ListViewItem items = new ListViewItem(tempPlayItem);//받는 즉시 바로 리스트뷰                
                    ...
                    char timeSpr = ':';
                    timeMax[maxCount-1] = Int16.Parse(tempPlayItem[2].Split(timeSpr)[0]) * 360 +
                       Int16.Parse(tempPlayItem[2].Split(timeSpr)[1]) * 60 + Int16.Parse(tempPlayItem[2].Split(timeSpr)[2]);
                    musicName[maxCount-1] = tempPlayItem[0];
        ...
        private void serverStop()//폼종료나 혹은 disconnect 상황에서 이거 실행
        {
            m_bStop = false;
            m_bCounnect = false;
            if (stream_alive)
        ...
        private void button1_Click(object sender, EventArgs e)//connect 버튼
        {
        ...
        private void track()//track 움직이는 함수
        {
            ...
            while (musicPlay)
            {
                ...
                if (WMP.controls.currentPosition >= timeMax[timeCount] - 1)
                {
                    if (musicPlayType == 1)
                    {
                        ...
                        else
                        {
                            ...
                            Thread.Sleep(1000);//조작을 그지가지 하는 경우를 방지하기 위해
                        }
                    }
                ...
                trackBar1.Value = (int)WMP.controls.currentPosition;//값만큼 트랙바 바꾸기
            }
        }

        private void button5_Click(object sender, EventArgs e)//재생 버튼
        {
            if (playMusicList.TopItem == null)
                return;
            if (!musicPlay)
            {
                if (playMusicList.FocusedItem == null)
                {
                ...
                else
                {
                    int temp = timeCount;
                    WMP.controls.play();
                    timeCount = playMusicList.FocusedItem.Index;
        ...
        private void trackBar1_Scroll(object sender, EventArgs e)//트랙바를 움직이면 노래를 바꾸자
        {
            WMP.controls.currentPosition = trackBar1.Value;
        }

        private void button7_Click(object sender, EventArgs e)//파일 삭제 버튼
        {
            string temp;
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)//타입결정 콤보박스
        {
        ...
        private void button9_Click(object sender, EventArgs e)//업로드할 파일
        {
            openFileDialog1 = new OpenFileDialog();
            openFileDialog1.ShowDialog();

            uplodaFileName = openFileDialog1.FileName;
            
            textBox4.Text = uplodaFileName;
        }

        private void button8_Click(object sender, EventArgs e)//업로드 버튼
        {
            if (button1.Text == "Connect")
